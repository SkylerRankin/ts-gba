...
const processLSR1 = (cpu: CPU, i: number) : ProcessedInstructionOptions => {
    ...
    let result;
    if (imm === 0) {
        cpu.setStatusRegisterFlag('c', isNegative32(rmValue));
        result = 0;
    } else {
        cpu.setStatusRegisterFlag('c', ((rmValue >>> (imm - 1)) & 0x1) === 1);
...
const processSTRH1 = (cpu: CPU, i: number) : ProcessedInstructionOptions => {
    ...
    const rd = i & 0x7;
    const address = cpu.getGeneralRegister(rn) + (imm * 2);
    const cycles = cpu.memory.setInt16(halfWordAlignAddress(address), cpu.getGeneralRegister(rd));
    cpu.cycles += cycles;
...
const processPUSH = (cpu: CPU, i: number) : ProcessedInstructionOptions => {
    ...
    for (let i = 0; i <= 7; i++) {
        if (((regList >>> i) & 0x1) === 1) {
            const cycles = cpu.memory.setInt32(address, cpu.getGeneralRegister(i));
            cpu.cycles += cycles;
            address += 4;
        }
    }
    ...
    if (r === 1) {
        ...
        const cycles = cpu.memory.setInt32(address, lr);
        cpu.cycles += cycles;
        address += 4;
    }
